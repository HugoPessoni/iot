...
#define DECIBEL_PIN 32  // Pino alterado para GPIO 32
...
void setup() {
  ...
  // Lista os arquivos na pasta /music
...
void playAudio() {
  const char* filePath = "/music/Jingle Bells.mp3";
  
  if (SD_MMC.exists(filePath)) {
    audio.connecttoFS(SD_MMC, filePath);
    isPlaying = true;  // Define a flag como true para indicar que o áudio está tocando
  } else {
    Serial.println("ERROR: File doesn't exist: /music/Jingle Bells.mp3");
  }
}

// Função para listar os arquivos no diretório especificado
